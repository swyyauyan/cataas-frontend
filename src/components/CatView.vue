    ...
    <div class="cat-view">
        <div class="cat-input">
            ...
            <div>
                ...
                <input v-model="catSay" type="text" id="catSay" name="catSay"><button v-on:click="getTalkingCat">Go!</button>
            </div>
        </div>
        <CatTable v-bind:catTableCol = "catTableCol" v-bind:catTableRow = "catTableRow" v-bind:catPhoto = "catPhoto"/>
    </div>
</template>
    ...
    getPictures: async function (event) {
      ...
      if (additionalCall > 0) {
        ...
        if (this.isTalkingCat) {
          apiUrl = process.env.VUE_APP_BACKEND_HOST + '/cats?text=' + this.catSay
