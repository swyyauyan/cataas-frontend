    ...
    <table class="cat-table">
      <tr v-for="(a, i) in +catTableRow" v-bind:key="i">
        <td v-for="(b, j) in +catTableCol" v-bind:key="j">
          <img
            @click="clickMethod(catPhoto[getCounter(b, i)])"
            class="cat-table-result"
            v-bind:src="getSrcLink(b, i)"
          />
        </td>
      </tr>
    </table>
...
export default {
  ...
  methods: {
    getSrcLink: function (b, i) {
      return this.catPhoto[this.getCounter(b, i)] || '/assets/cat_wait.gif'
    },
    getCounter: function (b, i) {
      return Number(b) + Number(i) * Number(this.catTableCol) - 1
    },
